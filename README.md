## Test MAPE: 1.86%
 I used XGBoost tree-based ML pipeline for tye given task.

 ## 1.Data Preparation & Anomaly Handling
 The raw national grid data contained severe undocumented spikes, missing timestamps, and potential data leakage risks. I employed following techniques to tackle these problems :-
  1. All data was forced into a strict hourly grid (.resample('H')). 
  2. All supply-side metrics (e.g., generation, fuel mix, load shedding) were completely dropped to prevent the model from "cheating" by learning the grid's post-facto dispatch physics.
  3. Anomaly Detection: Rather than using a standard Z-score (which gets skewed by massive outliers), a 24-hour Rolling Median and Interquartile Range (IQR) was used. A conservative multiplier of 3.0 was applied to flag extreme undocumented spikes without erasing natural, legitimate demand peaks.
  4. Missing Data Handling: Flagged anomalies and missing gaps were replaced with NaN rather than interpolated. Because XGBoost utilizes a sparsity-aware split-finding algorithm, it naturally learns the best direction to route missing values. This prevents the bias that linear interpolation introduces during volatile grid events.

 ## 2. Feature Engineering 
 To convert the data into a trainable features i employed following techniques
  1. Time Encoding: Features like hour_of_day , month were transformed using Sine and Cosine functions to maps time onto a continuous circle, allowing the model to mathematically understand that Hour 23 smoothly transitions into Hour 0.
 ...
 Model was trained using an expanding-window TimeSeries cross-validation to tune hyperparameters safely. 
